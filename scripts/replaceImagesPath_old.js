...
if (args && args.length > 0 && args[3]) {
	...
	// Replace all image links with references to assets/images
	content = content.replace(imageRegex, (match, captureGroup) => {
		// Extract the image file name from the URL
		const imageName = captureGroup.split('/').pop();
		// Replace the image link with the assets/images reference
