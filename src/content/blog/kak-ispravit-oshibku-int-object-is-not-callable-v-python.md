...
description: "Ошибка TypeError: int'object is not callable возникает, когда вы пытаетесь вызвать целое число (объект int), как если бы это была функция!"
...
tags:
  - Python
draft: false
lastmod: 2024-04-23T14:25:37.375Z
---
...
Все встроенные функции определяются в модуле builtins и получают глобальное имя для облегчения доступа. Например, sum() относится к функции __builtins__.sum().
...
2. Явно обратиться к функции sum() из модуля builtins (__bultins__.sum).
